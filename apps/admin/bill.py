...
from apps.base.models  import Bill
...
def list(request):
    ...
    res  = []
    _res = DB.bill.find(query).sort(sort).skip((page-1)*limit).limit(limit)
    for _r in _res:
        _r['id'] = _r['_id']
    ...
    paginator = Paginator(DB.bill.find(query,fileds=['_id']),limit)
...
def delete(request,oid):
    ...
    DB.bill.remove({'_id':ObjectId(oid)})
...
def edit(request,oid):
    ...
    if request.method == 'GET':
        building = DB.bill.find_one({'_id':oid})
    ...
    account = Bill.objects.get(id=oid)
