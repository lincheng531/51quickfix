	 	...
	 	for r in range(2, nrows):
	 		...
	 		for sb in sbs:
		 		for xh in xhs:
			 		...
			 		if not brand:
			 			brand_id = DB.brand.save({'name':pp, 'initial':get_pinyin_initials(pp), 'create_time':dt.now(), 'create_time':dt.now()})
			 		else:
			 			brand_id = brand['_id']
			 		bkc = DB.product.find_one({'name':sb.strip(), 'brand':brand_id})
			 		if not bkc:
			 			print tab, r, sb, xh, brand_id
			 		else:
						...
						if not DB.spare.find_one({'product_name':sb, 'no':items.get('no'), 'name':items.get('name'), 'brand':items.get('brand'), 'model':xh}) and items.get('name'):
